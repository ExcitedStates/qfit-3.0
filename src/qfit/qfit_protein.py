...
class QFitProtein:
    ...
    def _run_qfit_residue_parallel(self):
        ...
        for residue in residues:
            ...
            if not os.path.exists(fname):
                logger.warn(
                    f"[{residue_directory}] Couldn't find multiconformer_residue.pdb!"
        ...
        if self.options.debug:
            ...
            if self.structure.scale or self.structure.crystal_symmetry:
                multiconformer_model.tofile(
                    fname, self.structure.scale, self.structure.crystal_symmetry
                )
            else:
    ...
    def _run_qfit_residue(residue, structure, xmap, options, logqueue):
        ...
        if os.path.exists(fname):
            logger.info(
                f"Residue {identifier}: {fname} already exists, using this checkpoint."
            )
            return

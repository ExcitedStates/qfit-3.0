"find/returns residues within specific distance of specified ligand"
import numpy as np
import pandas as pd
import argparse
import logging
...
def select_close_residues(holo_str, apo_str, ligand, dist, holo_pdb_name, apo_pdb_name, write):
        neighbors_apo = {}
        neighbors_holo = {}
        for coor in ligand.coor:
            print(coor)
            dist_holo = np.linalg.norm(holo_str.coor - coor, axis=1)
            dist_apo = np.linalg.norm(apo_str.coor - coor, axis=1)
            for near_residue, near_chain in zip(holo_str.resi[dist_holo < dist], holo_str.chain[dist_holo < dist]):
                    key_holo = str(near_residue)+" "+near_chain
                    if key_holo not in neighbors_holo:
                        neighbors_holo[key_holo]=0
            for near_residue, near_chain in zip(apo_str.resi[dist_apo < dist], apo_str.chain[dist_apo < dist]):
                    key_apo = str(near_residue)+" "+near_chain
                    if key_apo not in neighbors_apo:
                        neighbors_apo[key_apo]=0
        with open(holo_pdb_name + '_' + str(dist) + '_closeresidue.txt', 'w') as file:
          for key in neighbors_holo.keys():
            print(key)
            residue_id,chain = key.split()
            file.write(chain + ',' + residue_id + "\n")
            if write == 1:
               residue = holo_str.extract(f"chain {chain} and resi {residue_id}")
               if not close_atoms_chain_holo==None:
                  close_atoms_chain_holo = close_atoms_chain_holo.combine(residue)
               else:
                  close_atoms_chain_holo = holo_str.extract(f'chain {chain} and resi {residue_id}')
        with open(apo_pdb_name + '_' + str(dist) + '_closeresidue.txt', 'w') as file:
          for key in neighbors_apo.keys():
            residue_id,chain = key.split()
            file.write(chain + ',' + residue_id + "\n")
            if write == 1:
               residue = apo_str.extract(f"chain {chain} and resi {residue_id}")
               if not close_atoms_chain_apo==None:
                  close_atoms_chain_apo = close_atoms_chain_apo.combine(residue)
               else: 
                  close_atoms_chain_apo = apo_str.extract(f'chain {chain} and resi {residue_id}')
         
def select_close_ligands(structure): #not currently being used in main
        dist_apo=np.linalg.norm(self.hetatoms_apo.coor[:][:]-self.lig_center, axis=1)
        mask_apo = dist_apo < args.distance
...
def main():
    ...
    ligand = holo_structure.extract('resn', args.lig)

    if args.write != None:
       write = 1
    else:
       write = 0
    select_close_residues(holo_receptor, apo_receptor, ligand, args.dist, args.holo_pdb_name, args.apo_pdb_name,write)  
    print(args.holo_pdb_name, args.apo_pdb_name, ligand)

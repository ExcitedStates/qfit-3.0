...
class QFitWater:
    ...
    def run(self):
        if self.options.pdb is not None:
            ...
            self.pdb = ""
        self.water = self.structure.extract(
            "resn", "HOH", "=="
        )  # create blank water object
        ...
        if len(self.water.altloc) > 1:
            ...
            if len(self.water.resi) > 1:
                logger.error(
                    "Duplicate water molecules found. Please run remove_duplicates.py"
                )
                return
        ...
        # Get a list of all residue numbers and associated chain numbers
        residue_chain_pairs = list(
            zip(
                self.structure.extract("record", "ATOM", "==").resi,
                self.structure.extract("record", "ATOM", "==").chain,
            )
        )
        unique_residue_chain_pairs = list(set(residue_chain_pairs))

        for res, chain in unique_residue_chain_pairs:
            print(res, chain)
            residue = self.structure.extract("resi", res, "==").extract(
                "chain", chain, "=="
            )
            ...
            qfit = QFitWater_Residue(
                residue, full_occ, xmap_reduced, self.water_holder, n, self.options
            )
            qfit.run()
            del xmap_reduced
            del qfit

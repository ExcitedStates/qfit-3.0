...
def main():
    ...
    fname = os.path.join(options.directory, pdb_id + f'multiconformer_{chainid}_{resi}.{pdb_ext}')
    if icode:
        fname = os.path.join(options.directory, pdb_id + f'multiconformer_{chainid}_{resi}_{icode}.{pdb_ext}')
    try:

...
class TestVolumeXmapIO(UnitBase):
    def test_xmap_from_mtz_water(self):
        ...
        print(f"MTZ: {MTZ}")
        xmap1 = XMap.fromfile(MTZ, label="FWT,PHIFWT", transformer="cctbx")
    ...
    @pytest.mark.skip(reason="FIXME Numbers need updating for new input file")
    def test_xmap_from_mtz_large(self):
        ...
        mtz_file = op.join(self.DATA_BIG, "composite_omit_map.mtz")
        xmap1 = XMap.fromfile(mtz_file, label="2FOFCWT,PH2FOFCWT", transformer="cctbx")
        _validate_map(xmap1)
        assert xmap1.is_canonical_unit_cell()
    ...
    def test_read_write_ccp4_map(self):
        ...
        map_file = op.join(self.TESTS_DIR, "qfit_io_test", "7o9m.map")
    ...
    def test_cctbx_io_with_fft(self):
        ...
        _validate_map(xmap1)
        xmap2 = XMap.fromfile(MTZ, label="FWT,PHIFWT", transformer="cctbx")
        _validate_map(xmap2)

    def test_canonical_unit_cell(self):
        tmp_mtz = self.make_tiny_fmodel_mtz()
        xmap_mtz = XMap.fromfile(tmp_mtz, label="FWT,PHIFWT", transformer="cctbx")
        ...
        # XXX the expand_to_p1 routine (using the C extension) does not
        # produce numerically identical results to the P1 map!
        assert np.all(corr[0][1] >= 0.9707) #0.99999999999)

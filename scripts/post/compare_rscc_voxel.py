...
def build_argparser():
    ...
    p.add_argument("structure", type=str, help="deposited PDB-file containing structure.")
...
def main():
  ...
  dep_xmap = dep_xmap.extract(gen_ligand.coor, padding=8) # Create a copy of the deposited map around the atomic coordinates provided.

  gen_xmap = XMap.fromfile(
    options.gen_map, label='2FOFCWT,PH2FOFCWT'
  )
  ...
  gen_xmap = gen_xmap.extract(gen_ligand.coor, padding=8) # Create a copy of the map around the atomic coordinates provided.
  ...
  gen_fname = os.path.join(
        options.directory, f"gen{ext}"
    )  # this should be an option

...
def parse_args():
    ...
    p.add_argument("pdb_files", type=str, help="pdb file to extract order parameters from")
...
def get_coords(pdb, r, a, ch):  # pdb, residue, atom, chain
    coords = {}
    weight = {}
    bfacts = {}
    sum_weight = 0.0
    ...
    if len(coords) == 0:
        print("  .. Warning: missing coodinates %i %s %s" % (r, a, pdb))
    coordslist = []
    weightlist = []
    bfactslist = []
...
def calc_S2(pdb_file_list, data_mat, res):
    ...
    b_mat = []
    with open(pdb_file_list, 'r') as file:
        pdb_files = file.read().splitlines()
    for d in data_mat:  # for everyline
        print('D:')
        print(d)
        coord1_all, weight1_all, bfact1_all = [], [], []
        coord2_all, weight2_all, bfact2_all = [], [], []
        S2ang = 0.0
        S2ortho = 1.0
        p2_list = []
        b_list = []
        ...
        for pdb in pdb_files:
            coord1, weight1, bfac1 = get_coords(pdb, d[0], d[1], d[6])  # pdb, residue, atom, chain
            coord2, weight2, bfac2 = get_coords(pdb, d[2], d[3], d[6])  # pdb, residue, atom, chain
            weight1 = [w / len(pdb_files) for w in weight1]
            weight2 = [w / len(pdb_files) for w in weight2]
        # Check and correct some simple data consitencies
            ...
            for i in range(len(p)):  # for every occupancy
                ...
                S2ortho -= (
                    (((bfac1[i] + bfac2[i]) / (8 * math.pi * math.pi)) * p[i]) * 5
                ) / (b*res)
...
if __name__ == "__main__":
    ...
    s2_calc, struc_mat, p2_mat, s2_ortho, s2_ang, prob_mat, b_mat = calc_S2(
        pdb_file_list, data_mat, resolution
    )
    ...
    output["s2ortho"] = output["s2ortho"].apply(lambda x: max(x, 0))

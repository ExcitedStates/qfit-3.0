...
class _Selector:
    ...
    def __init__(self):
        ...
        identifyer_selection = identifyer_selectors + identifyers.setParseAction(self._push)
        resi_selection = kw_resi + (arange + pp.ZeroOrMore(plus + arange)).setParseAction(self._push)
        numeric_selection = numeric_selectors + (operators + floatnumber).setParseAction(self._push_first)
        ...
        expression << factor + pp.ZeroOrMore((kw_and_or + factor).setParseAction(self._push_first))
